...
export const invoices = pgTable(
  "invoices",
  {
    ...
    // Unique per hotel (was global) — keeps each hotel's GST invoice
    // sequence independent. See uq_invoices_property_number.
  ...
  (t) => ({
    propertyNumberUnique: uniqueIndex("uq_invoices_property_number").on(
      t.propertyId,
      t.invoiceNumber,
    ),
  }),
...
export const payments = pgTable(
  ...
  {
    id: uuid("id").primaryKey().defaultRandom(),
    // Unique per hotel (was global) — see uq_payments_property_receipt.
  ...
  (t) => ({
    propertyReceiptUnique: uniqueIndex("uq_payments_property_receipt")
      .on(t.propertyId, t.receiptNumber)
    ...
    // with no advance. The `IS NOT NULL` check (not `>= 0`) allows refund rows.
