// LiquidEther — GPU fluid-simulation backdrop (React Bits, MIT).
//
// Renders a WebGL Navier-Stokes sim whose velocity field is mapped through a
// colour palette. Used behind the auth pages' forest brand pane, so the
// palette here is Stayvia's (jade / brass), not the upstream purple.
//
// Cost note: this is the only thing in the app that pulls `three` (~800 kB).
// It is therefore imported LAZILY by the auth pages — React.lazy keeps three
// out of the main app chunk so signed-in staff never download it.
//
// Behaviour: pauses via IntersectionObserver when scrolled out of view and
// via visibilitychange when the tab is hidden, so it never burns GPU in the
// background. Respects prefers-reduced-motion by not mounting at all (see
// the auth pages' guard).
